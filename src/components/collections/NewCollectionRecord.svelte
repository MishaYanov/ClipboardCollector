<script lang="ts">
    ...
    import { PortName, PopupToBackGroundMessageType, type ICollection } from '../../models';
  import { generateTimestamp } from '../../utils';
    ...
    const submitForm = (event: Event) => {
      ...
      // Optionally, handle immediate UI updates or wait for background confirmation
      // Here, we rely on the background script to send back a confirmation message
    };
    ...
    ps.onMessage((message: any) => {
      if (message.type === PopupToBackGroundMessageType.COLLECTION_RECORD_ADDED) {
      ...
      } else if (message.type === PopupToBackGroundMessageType.ERROR) {
        if (message.payload?.collectionId === collection.id) {
          errorMessage = message.payload.error || 'An error occurred while adding the record.';
          addingRecord = false;
        }
      }
    });
  ...
  <section class="add-record">
    ...
    <form on:submit={submitForm}>
      <label for="record-text">Text<span class="required">*</span></label>
      <input
        type="text"
        id="record-text"
        name="record-text"
